...
void FeatureExtraction::LidarHandler(const sensor_msgs::PointCloud2::ConstPtr& lidar_msg) {
  if (!CachePointCloud(lidar_msg)) {
    return;
  }
  CloudExtraction();
  CaculateSmoothness();
...
void FeatureExtraction::LidarHandler(RTPointCloud::Ptr raw_cloud, RTPointCloud::Ptr undistort_cloud) {
...
// 是否有ring通道
bool FeatureExtraction::CheckMsgFields(const sensor_msgs::PointCloud2& cloud_msg, std::string fields_name) {
  bool flag = false;
...
void FeatureExtraction::ProjectOrganizedCloud(
  ...
  for (size_t column_id = 0; column_id < cur_cloud->width; ++column_id) {
    for (size_t row_id = 0; row_id < cur_cloud->height; ++row_id) {
      ...
      if (!std::isfinite(p.x) || !std::isfinite(p.y) || !std::isfinite(p.z))  //原本是pcl_isfinite
        continue;
...
void FeatureExtraction::ProjectPointCloud(const RTPointCloud::Ptr cur_cloud, cv::Mat& dist_image,
					    RTPointCloud::Ptr corresponding_cloud) {
